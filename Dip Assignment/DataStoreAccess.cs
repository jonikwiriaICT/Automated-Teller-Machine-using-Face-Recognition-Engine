...
namespace Dip_Assignment
{
    class DataStoreAccess:IDataStoreAccess
    {
        ...
        public DataStoreAccess(String databasePath)
        {
            ...
            _sqLiteConnection = new SqlConnection("Data Source=CIT-VUNA\\SQLEXPRESS;Initial Catalog=FaceMatchingATM;Integrated Security=True;");
        }
        ...
        {
            _sqLiteConnection.Open();
            SqlCommand cmd = new SqlCommand("select count(*) from Customer where username=@name and PIN=@PIN",_sqLiteConnection);
            cmd.Parameters.Add(new SqlParameter("@name",Global.UserName));
            cmd.Parameters.Add(new SqlParameter("@PIN", Global.PIN));
